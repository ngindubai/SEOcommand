    ...
    <div className="space-y-5">
      ...
      <Card className="p-4">
        ...
        <p className="text-2xs text-muted">
          ...
          <code className="rounded bg-workspace px-1 py-0.5 text-[12px] text-ink">
            POST /api/sync
          </code>{" "}
  ...
  const columns = useMemo<Column<AppUser>[]>(
    () => [
      ...
      },
      {
        key: "permissions",
        header: "Access scope",
        render: (u) => <div className="flex flex-wrap gap-1">{u.source === "bootstrap" ? <span className="text-xs text-muted">Bootstrap portfolio account</span> : u.grants.map((grant,index) => <span key={`${grant.scopeType}:${grant.scopeId}:${index}`} className="rounded-full bg-workspace px-2 py-1 text-[12px] font-semibold text-muted">{grant.scopeType}{grant.scopeId ? ` · ${groups.find((group) => group.id === grant.scopeId)?.name ?? sites.find((site) => site.id === grant.scopeId)?.name ?? grant.scopeId}` : ""}</span>)}</div>,
      },
    ],
    [groups, sites],
  );
