    ...
    <div className="grid gap-5 xl:grid-cols-[.85fr_1.5fr]">
      ...
      <Card className={cn("min-w-0", styles.panel, styles.engagement)}><PanelHeading icon={Activity} title="Sessions & Engagement" subtitle={`Organic search · ${period(sessions.start, sessions.end)}${sessions.snapshotOnly ? " · saved totals" : ""}`} href={link("/research")} />
        <div className="px-5 pb-4 pt-4"><div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-5">{([
          ...
          {!sessions.snapshotOnly && <TrendChart rows={sessionRows} dataKey={sessionMetric} color="rgb(var(--section-color))" compare={sessions.comparable} />}<p className="mt-3 text-xs text-muted">{ga ? `${ga.domainIds.length} ${ga.domainIds.length === 1 ? "website" : "websites"} included. ${sessions.availableDays < days ? `${sessions.availableDays} of ${days} days available. ` : ""}${sessions.comparable ? "Dashed line: previous period." : "No complete comparison period."}` : "Saved Analytics totals are shown above. Daily history and breakdowns need their first successful collection. Review the website’s Google connection or refresh from Scan Centre."}</p><Source ds={d?.ga4_dashboard ?? d?.ga4_overview} label="Google Analytics" />
        </div>
      </Card>
    </div>
    {ga && <>
