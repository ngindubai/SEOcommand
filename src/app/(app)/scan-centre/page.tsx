...
export default function ScanCentrePage() {
  ...
  return <div>
    ...
    <section className="mb-5 overflow-hidden rounded-xl border border-border bg-[#272624] text-white shadow-card">
      <div className="grid gap-6 px-6 py-6 lg:grid-cols-[1fr_auto] lg:items-end"><div>
        ...
        <div><div className="text-[12px] uppercase tracking-wide text-white/45">Estimated call cost</div><div className="mt-0.5 text-xl font-black tnum">{(loading && !data ? "—" : money(estimate))}</div></div><div className="h-9 w-px bg-white/10" />
        <Button variant="primary" disabled={loading || !data || running || selected.length === 0 || blocked} onClick={() => void run()} className="bg-[#6B5BFF] hover:bg-[#7B6DFF]">{running ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}{selected.length === data?.modules.length ? "Run full scan" : `Run ${selected.length} tools`}</Button>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]"><div>
      ...
      {loading && !data ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 9 }, (_, index) => <div key={index} className="h-36 animate-pulse rounded-lg border border-border bg-card" />)}</div> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{data?.modules.map((module) => {
        ...
        return <button key={module.id} onClick={() => setSelected((current) => current.includes(module.id) ? current.filter((id) => id !== module.id) : [...current, module.id])} className={cn("group relative overflow-hidden rounded-lg border bg-card p-4 text-left shadow-card transition-all hover:-translate-y-0.5 hover:shadow-pop", active ? "ring-2 ring-offset-0" : "border-border")} style={{ borderColor: active ? module.color : undefined, "--tw-ring-color": `${module.color}20` } as React.CSSProperties}>
          ...
          <div className="mt-3 text-sm font-bold text-ink">{module.label}</div><p className="mt-1 min-h-9 text-xs leading-4 text-muted">{module.description}</p><div className="mt-3 flex items-center justify-between text-[12px] font-bold uppercase tracking-wide"><span className={module.paid ? "text-[#B9791A]" : "text-success"}>{module.paid ? "Provider call" : "Free check"}</span><span className="text-muted">{module.paid ? money(module.estimatedUsd) : "$0"}</span></div>
          <div className="mt-3 border-t border-border pt-2.5 text-xs leading-5"><span className="flex items-center gap-1.5 text-muted"><Clock3 className="h-3 w-3" aria-hidden="true" />Last updated</span>{module.lastUpdatedAt || module.lastUpdatedDate ? <time dateTime={module.lastUpdatedAt ?? module.lastUpdatedDate!} className="font-medium text-ink">{updatedLabel(module)}</time> : <span className="text-muted">Not updated yet</span>}</div>
        </button>;
      })}</div>}
    </div><Card className="h-fit overflow-hidden"><div className="flex items-center justify-between border-b border-border px-5 py-4"><div><h2 className="text-sm font-bold text-ink">Recent runs</h2><p className="mt-0.5 text-2xs text-muted">Live queue and completed evidence</p></div><button aria-label="Refresh scans" onClick={() => void load()} className="rounded-md p-2 text-muted hover:bg-workspace"><RefreshCcw className={cn("h-4 w-4", loading && "animate-spin")} /></button></div><div className="max-h-[720px] divide-y divide-border overflow-y-auto">
      {!loading && !error && !data?.jobs.length && <div className="p-4"><EmptyState icon={<Database className="h-6 w-6" />} title="No scans yet" description="Select the evidence you need and run the first refresh." /></div>}
      {data?.jobs.map((job) => { const browserCrawl = job.kind === "browser_crawl"; const modules = (Array.isArray(job.progress.modules) ? job.progress.modules : browserCrawl ? ["technical"] : []) as ScanModule[]; const active = ["queued", "running"].includes(job.status); return <div key={job.id} className="p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">{modules.map((module) => { const meta = data.modules.find((item) => item.id === module); const Icon = ICONS[module]; return <Link title={`Open ${meta?.label ?? module}`} key={module} href={`${browserCrawl ? "/technical-crawler" : RESULT_LINKS[module]}?site=${siteSlug}`} className="flex h-7 w-7 items-center justify-center rounded-md border border-border hover:bg-workspace" style={{ color: meta?.color }}><Icon className="h-3.5 w-3.5" /></Link>; })}</div>
        {job.status === "running" && <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-workspace"><div className="h-full w-1/2 animate-pulse rounded-full bg-gradient-to-r from-[#335CFF] to-[#12B8C4]" /></div>}{job.status === "queued" && <p className="mt-3 text-xs text-muted">{browserCrawl ? "Waiting for the hourly browser worker." : "Waiting to start. Data scans can start now."}{job.lastError && job.runAfter ? ` Retry eligible after ${new Date(job.runAfter).toLocaleString()}.` : ""}</p>}{job.lastError && <EvidenceMessage detail={job.lastError} />}
