...
export default function ScanCentrePage() {
  ...
  async function manage(jobId: string, action: "cancel" | "retry") {
    ...
    try {
      ...
      setMessage(action === "retry" ? "Scan queued again. Existing evidence is retained." : "Scan cancellation requested.");
      await load();
  ...
  return <div>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]"><div>
      ...
      {data?.jobs.map((job) => { const modules = (Array.isArray(job.progress.modules) ? job.progress.modules : []) as ScanModule[]; const active = ["queued", "running"].includes(job.status); return <div key={job.id} className="p-4">
        ...
        {active && <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-workspace"><div className="h-full w-1/2 animate-pulse rounded-full bg-gradient-to-r from-[#335CFF] to-[#12B8C4]" /></div>}{job.lastError && <EvidenceMessage detail={job.lastError} />}
        <div className="mt-3 flex items-center justify-between"><span className="text-[12px] text-muted">{job.requestedBy ? `By ${job.requestedBy}` : "Scheduled"}</span>{active ? <button disabled={Boolean(managing)} onClick={() => void manage(job.id, "cancel")} className="text-xs font-semibold text-critical">Cancel</button> : ["failed", "cancelled"].includes(job.status) ? <button disabled={Boolean(managing)} onClick={() => void manage(job.id, "retry")} className="text-xs font-semibold text-purple">Retry all tools · ~{money(data.modules.filter((item) => modules.includes(item.id)).reduce((sum, item) => sum + item.estimatedUsd, 0))}</button> : <Link href={`/sites/${siteSlug}`} className="flex items-center gap-1 text-xs font-semibold text-purple">Website overview <ChevronRight className="h-3 w-3" /></Link>}</div>
      </div>; })}
    </div></Card></div>
  </div>;
}
