...
export function qualifyRecommendations(bundle: DomainLiveBundle): DerivedRecommendation[] {
  return (bundle.datasets.recommendations?.data ?? []).map((recommendation) => {
    ...
    return { ...recommendation,
      ...
      estImpact: top.length ? `Highest measured demand: ${top.map((row) => `${row.query} (position ${row.position}, ${row.impressions} impressions)`).join("; ")}. No traffic gain is guaranteed.` : "Collect current query evidence before approving this work.",
    };
  });
}
