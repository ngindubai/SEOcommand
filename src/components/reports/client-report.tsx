...
import { analyticsPeriod, searchPeriod, shippedInPeriod } from "@/lib/reporting";
...
function compact(value: number | null | undefined): string {
  ...
  return Math.abs(value) >= 1_000 ? compactNumber(value) : new Intl.NumberFormat("en-US", { maximumFractionDigits: value >= 100 ? 0 : 1 }).format(value);
}
...
function Delta({ value, invert = false }: { value: number | null; invert?: boolean }) {
  if (value == null || Math.abs(value) < 0.05) return <span className="inline-flex items-center gap-1 text-[12px] font-bold text-[#65708A]"><Minus className="h-3 w-3" />No comparison</span>;
  const positive = invert ? value < 0 : value > 0;
  const Icon = value > 0 ? ArrowUpRight : ArrowDownRight;
  return <span className={`inline-flex items-center gap-1 text-[12px] font-bold ${positive ? "text-[#087A57]" : "text-[#C43D46]"}`}><Icon className="h-3 w-3" />{Math.abs(value).toFixed(1)}%</span>;
}
...
function ReportStat({ label, value, detail, delta, invert }: { label: string; value: string; detail: string; delta?: number | null; invert?: boolean }) {
  return <div className="border-t-2 border-[var(--report-accent)] bg-white px-4 py-4 shadow-[0_1px_0_rgba(17,24,43,0.06)]"><div className="text-[12px] font-extrabold uppercase tracking-[0.16em] text-[#65708A]">{label}</div><div className="mt-2 font-serif text-[27px] font-bold leading-none text-[#11182B] tnum">{value}</div><div className="mt-2 flex min-h-4 items-center justify-between gap-2 text-[12px] text-[#65708A]"><span>{detail}</span>{delta !== undefined && <Delta value={delta ?? null} invert={invert} />}</div></div>;
}

function Section({ eyebrow, title, summary, children }: { eyebrow: string; title: string; summary?: string; children: ReactNode }) {
  return <section className="report-section border-t border-[#DDE2EC] pt-7"><div className="grid gap-5 lg:grid-cols-[190px_minmax(0,1fr)]"><div><div className="text-[12px] font-extrabold uppercase tracking-[0.18em] text-[var(--report-accent)]">{eyebrow}</div><h2 className="mt-2 font-serif text-2xl font-bold leading-tight text-[#11182B]">{title}</h2>{summary && <p className="mt-3 text-[12px] leading-5 text-[#65708A]">{summary}</p>}</div><div className="min-w-0">{children}</div></div></section>;
}

function ChartShell({ title, note, children }: { title: string; note: string; children: ReactNode }) {
  return <div className="report-chart rounded-md border border-[#DDE2EC] bg-white p-4"><div className="mb-4 flex items-baseline justify-between gap-3"><h3 className="text-xs font-extrabold text-[#11182B]">{title}</h3><span className="text-[12px] text-[#7B8498]">{note}</span></div>{children}</div>;
}

const tick = { fontSize: 9, fill: MUTED };
function ReportTooltip({ active, payload, label }: any) {
  if (!active || !payload?.length) return null;
  return <div className="rounded border border-[#DDE2EC] bg-white px-3 py-2 text-[12px] shadow-lg"><div className="mb-1 font-bold text-[#11182B]">{typeof label === "string" ? dateLabel(label) : label}</div>{payload.map((item: any) => <div key={item.dataKey} className="flex items-center justify-between gap-5 text-[#65708A]"><span>{item.name}</span><span className="font-bold text-[#11182B]">{compact(Number(item.value))}</span></div>)}</div>;
}
...
function ReportTable({ headers, rows }: { headers: string[]; rows: ReactNode[][] }) {
  return <div className="overflow-hidden rounded-md border border-[#DDE2EC]"><table className="w-full table-fixed border-collapse"><thead><tr className="bg-[#F2F4F8]">{headers.map((header, index) => <th key={header} className={`px-3 py-2.5 text-[12px] font-extrabold uppercase tracking-[0.11em] text-[#65708A] ${index ? "text-right" : "text-left"}`}>{header}</th>)}</tr></thead><tbody>{rows.map((row, rowIndex) => <tr key={rowIndex} className="border-t border-[#E6E9F0]">{row.map((cell, cellIndex) => <td key={cellIndex} className={`px-3 py-2.5 text-[12px] leading-4 text-[#11182B] ${cellIndex ? "text-right tnum" : "text-left font-semibold"}`}>{cell}</td>)}</tr>)}</tbody></table></div>;
}

function NoData({ label }: { label: string }) {
  return <div className="rounded-md border border-dashed border-[#CBD1DE] bg-[#F7F8FB] px-4 py-5 text-center text-[12px] text-[#65708A]">{label} will appear after the connected data source completes its first sync.</div>;
}
...
export type ReportOutcome = { id: string; title: string; module: string; shippedAt?: string | null; proof?: { shipped: boolean; verified?: boolean }; verification: { shipment?: { recordedAt?: string }; outcome?: string; confidence?: string; valueCreated?: { amount: number; currency: string; method: string } | null }; metrics: Array<{ label: string; percentChange: number | null }> };
export function ClientReport({ site, template, branding, bundle, outcomes: allOutcomes = [], days = 28 }: { site: Domain; template: ReportTemplate; branding: ReportBranding; bundle: DomainLiveBundle; outcomes?: ReportOutcome[]; days?: number }) {
  const search = searchPeriod(bundle, days);
  const analytics = analyticsPeriod(bundle, days);
  const gsc = search.total;
  const gscSeries = search.current;
  const ga4 = analytics.total;
  const outcomes = shippedInPeriod(allOutcomes, search.start, search.end);
  ...
  const periodStart = search.start;
  const periodEnd = search.end;
  ...
  }).filter((row) => ai.some((prompt) => prompt.platforms.some((platform) => platform === (row.platform === "ChatGPT" ? "chatgpt" : row.platform === "Google AI" ? "google_ai" : row.platform.toLowerCase()))));
  const insights: string[] = [];
  if (clickChange != null) insights.push(`Organic clicks ${clickChange >= 0 ? "increased" : "decreased"} ${Math.abs(clickChange).toFixed(1)}% compared with the preceding equivalent period.`);
  ...
  return <article id="client-report" className="client-report mx-auto max-w-[940px] overflow-hidden bg-[#F7F8FB] text-[#11182B] shadow-[0_24px_80px_rgba(17,24,43,0.12)]" style={{ "--report-accent": branding.accent, "--report-secondary": branding.secondaryColor } as CSSProperties}>
    <section className="report-cover relative flex min-h-[680px] flex-col overflow-hidden bg-[#11182B] px-10 py-10 text-white sm:px-14 sm:py-12">
      ...
      <div className="relative flex items-start justify-between gap-6">
        ...
        <div className="text-right text-[12px] font-bold uppercase tracking-[0.18em] text-white/45">Confidential<br />Client report</div>
      </div>
      <div className="relative mt-auto max-w-2xl pb-12">
        <div className="mb-6 h-1 w-20" style={{ background: branding.secondaryColor }} />
        <div className="text-[12px] font-extrabold uppercase tracking-[0.22em] text-white/45">{template.type} · {site.host}</div>
      ...
      <div className="relative grid gap-5 border-t border-white/10 pt-6 text-[12px] sm:grid-cols-3"><div><div className="uppercase tracking-[0.15em] text-white/35">Reporting period</div><div className="mt-1 font-bold text-white/80">{period}</div></div><div><div className="uppercase tracking-[0.15em] text-white/35">Prepared by</div><div className="mt-1 font-bold text-white/80">{branding.preparedBy}</div></div><div><div className="uppercase tracking-[0.15em] text-white/35">Issued</div><div className="mt-1 font-bold text-white/80">{generated}</div></div></div>
    </section>
    ...
    <div className="report-body space-y-10 px-6 py-10 sm:px-10 sm:py-12">
      <Section eyebrow="Executive reading" title="Performance at a glance" summary="The most important findings from the latest connected search, analytics and technical datasets.">
        <div className="rounded-md bg-[#11182B] p-5 text-white"><div className="text-[12px] font-extrabold uppercase tracking-[0.18em] text-white/45">Executive summary</div><div className="mt-3 space-y-2.5">{insights.map((insight) => <div key={insight} className="flex gap-3 text-[12px] leading-5 text-white/75"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[var(--report-secondary)]" />{insight}</div>)}</div></div>
      </Section>
      ...
      <div className="grid grid-cols-2 gap-px overflow-hidden rounded-md bg-[#DDE2EC] lg:grid-cols-4">
        ...
        <ReportStat label="Organic sessions" value={compact(ga4?.sessions)} detail={analytics.snapshotOnly ? `Saved period: ${analytics.start ?? "unknown"} – ${analytics.end ?? "unknown"}` : "Google Analytics"} />
        ...
        <ReportStat label="Technical health" value={onpage ? `${Math.round(onpage.healthScore)}/100` : "—"} detail={onpage ? `${onpage.crawlRun?.pagesCrawled ?? "Unverified"} pages assessed` : "Awaiting crawl"} />
        <ReportStat label="AI mention rate" value={pct(aiMentionRate)} detail={`${ai.length} latest prompt records · ${bundle.datasets.ai_prompts?.capturedOn ?? "date unavailable"}`} />
      </div>

      {performanceReport && <Section eyebrow="01 · Search demand" title="Organic performance" summary={`Search Console: ${search.availableDays} of ${days} days available. Analytics and dimensional tables use their saved snapshots where daily history is unavailable.`}>
        <div className="space-y-4">
          {gscSeries.length ? <ChartShell title="Search demand and response" note={`${search.start ?? "—"} – ${search.end ?? "—"}`}><SearchPerformanceChart data={gscSeries} accent={branding.accent} secondary={branding.secondaryColor} /></ChartShell> : <NoData label="Daily search performance" />}
          {(queries.length || channels.length) ? <div className="grid gap-4 xl:grid-cols-2">
            {queries.length ? <div><div className="mb-2 text-[12px] font-extrabold uppercase tracking-[0.16em] text-[#65708A]">Leading search queries</div><ReportTable headers={["Query", "Clicks", "Position"]} rows={[...queries].sort((a, b) => b.clicks - a.clicks).slice(0, 6).map((row) => [row.key, compact(row.clicks), row.position.toFixed(1)])} /></div> : <NoData label="Search query detail" />}
            {channels.length ? <div><div className="mb-2 text-[12px] font-extrabold uppercase tracking-[0.16em] text-[#65708A]">Traffic contribution</div><ReportTable headers={["Channel", "Sessions", "Conversions"]} rows={[...channels].sort((a, b) => b.sessions - a.sessions).slice(0, 6).map((row) => [row.channel, compact(row.sessions), compact(row.conversions)])} /></div> : <NoData label="Traffic channel detail" />}
          </div> : null}
          {pages.length ? <><div className="pt-2 text-[12px] font-extrabold uppercase tracking-[0.16em] text-[#65708A]">Leading organic landing pages</div><ReportTable headers={["Landing page", "Sessions", "Conversions", "Engagement"]} rows={[...pages].sort((a, b) => b.sessions - a.sessions).slice(0, 7).map((page) => [page.landingPage, compact(page.sessions), compact(page.conversions), pct(page.engagementRate)])} /></> : null}
        </div>
      </Section>}

      {rankingReport && <Section eyebrow="02 · Market position" title="Rankings and visibility" summary={`Ranking and movement tables are saved snapshots, independent of the search reporting period. Ranking snapshot: ${bundle.datasets.rank_snapshots?.capturedOn ?? "unavailable"}.`}>
      ...
      {technicalReport && <Section eyebrow="03 · Technical quality" title="Crawl health and risk" summary="Issues are ranked by severity and affected-page count so remediation can begin with material search risk.">
        <div className="space-y-4">{onpage ? <><div className="grid gap-4 xl:grid-cols-[0.8fr_1.2fr]"><div className="flex min-h-[260px] flex-col justify-between rounded-md p-6 text-white" style={{ background: branding.accent }}><div><div className="text-[12px] font-extrabold uppercase tracking-[0.18em] text-white/55">Technical health</div><div className="mt-3 font-serif text-6xl font-bold">{Math.round(onpage.healthScore)}</div><div className="text-xs text-white/65">out of 100</div></div><div className="border-t border-white/20 pt-4 text-[12px] text-white/70">{onpage.crawlRun?.pagesCrawled ?? "Unverified"} pages crawled · {onpage.crawlRun?.newIssues ?? 0} new · {onpage.crawlRun?.resolvedIssues ?? 0} resolved</div></div><ChartShell title="Open issues by severity" note="Latest crawl"><SimpleBarChart data={technicalCounts} xKey="severity" bars={[{ key: "issues", name: "Issues", color: branding.secondaryColor }]} /></ChartShell></div><ReportTable headers={["Priority issue", "Severity", "Pages", "Recommended action"]} rows={[...onpage.issues].filter((issue) => issue.status !== "resolved").sort((a, b) => severityRank(b) - severityRank(a) || b.affectedPages - a.affectedPages).slice(0, 8).map((issue) => [issue.title, <span key={issue.id} className={`font-bold uppercase ${issue.severity === "critical" || issue.severity === "high" ? "text-[#C43D46]" : "text-[#9A6B08]"}`}>{issue.severity}</span>, compact(issue.affectedPages), issue.recommendedFix])} /></> : <NoData label="Technical crawl evidence" />}</div>
      </Section>}
      ...
      {aiReport && <Section eyebrow="05 · AI discovery" title="AI visibility" summary="Mention and citation rates show whether tracked answer engines recognise the brand and use its website as evidence.">
        <div className="space-y-4"><div className="grid grid-cols-2 gap-px overflow-hidden rounded-md bg-[#DDE2EC]"><ReportStat label="Mention rate" value={pct(aiMentionRate)} detail={`${ai.length} latest prompt records`} /><ReportStat label="Citation rate" value={pct(aiCitationRate)} detail="Owned-source citations" /></div>{aiPlatforms.length ? <ChartShell title="Visibility by answer engine" note="Latest recorded result for each prompt"><SimpleBarChart data={aiPlatforms} xKey="platform" bars={[{ key: "mentions", name: "Mentions %", color: branding.accent }, { key: "citations", name: "Citations %", color: branding.secondaryColor }]} /></ChartShell> : <NoData label="AI prompt visibility" />}{ai.length ? <ReportTable headers={["Tracked prompt", "Mention", "Citation", "Avg. recommendation"]} rows={[...ai].sort((a, b) => b.mentionRate - a.mentionRate).slice(0, 7).map((row) => [row.prompt, pct(row.mentionRate), pct(row.citationRate), row.avgPosition == null ? "—" : `#${row.avgPosition.toFixed(1)}`])} /> : null}</div>
      </Section>}

      {allOutcomes.length > 0 && <Section eyebrow="Measured delivery" title="Delivery and measured outcomes" summary="Only actions with shipment evidence dated within the search reporting period are included. Delivery and verified success are counted separately."><div className="space-y-3"><div className="grid grid-cols-3 gap-px overflow-hidden rounded-md bg-[#DDE2EC]"><ReportStat label="Shipped actions" value={String(outcomes.length)} detail={outcomes.length ? "With dated shipment evidence" : "None recorded in this period"} /><ReportStat label="Verified wins" value={String(outcomes.filter((item) => item.verification.outcome === "won").length)} detail="Measured outcomes" /><ReportStat label="Value created" value={outcomes.filter((item) => item.verification.valueCreated).map((item) => `${item.verification.valueCreated!.currency} ${compact(item.verification.valueCreated!.amount)}`).join(" + ") || "—"} detail="Recorded + estimated" /></div><ReportTable headers={["Delivered action", "Outcome", "Primary change", "Confidence"]} rows={outcomes.slice(0, 8).map((item) => [item.title, (item.verification.outcome ?? "Awaiting data").replace(/_/g, " "), item.metrics[0]?.percentChange == null ? "—" : `${item.metrics[0].percentChange! > 0 ? "+" : ""}${item.metrics[0].percentChange!.toFixed(1)}% ${item.metrics[0].label}`, item.verification.confidence ?? "—"])} /></div></Section>}
      ...
      <Section eyebrow="Next reporting cycle" title="Priorities and actions" summary="These recommendations are generated from the latest stored evidence and ranked by expected impact, confidence and effort.">
        {recommendations.length ? <div className="space-y-2.5">{recommendations.slice(0, 7).map((item, index) => <div key={item.id} className="grid gap-3 rounded-md border border-[#DDE2EC] bg-white p-4 sm:grid-cols-[34px_minmax(0,1fr)_110px]"><div className="flex h-8 w-8 items-center justify-center rounded-full text-xs font-black text-white" style={{ background: index < 2 ? branding.accent : "#8A93A7" }}>{index + 1}</div><div><div className="text-xs font-extrabold text-[#11182B]">{item.title}</div><div className="mt-1 text-[12px] leading-4 text-[#65708A]">{item.evidence}</div><div className="mt-2 text-[12px] font-bold uppercase tracking-[0.12em] text-[var(--report-accent)]">{item.estImpact}</div></div><div className="text-right text-[12px] text-[#65708A]"><div className="font-black text-[#11182B]">Priority {item.priorityScore}</div><div className="mt-1 capitalize">{item.confidence} confidence</div><div className="mt-1">Effort {item.effort}</div></div></div>)}</div> : <NoData label="Evidence-backed recommendations" />}
      </Section>

      <div className="flex items-center justify-between gap-4 border-t border-[#DDE2EC] pt-5 text-[12px] text-[#7B8498]"><span>{branding.footerText}</span><span className="shrink-0">{branding.contactEmail}{branding.showPoweredBy ? `${branding.contactEmail ? " · " : ""}Powered by SEOcommand` : ""}</span></div>
    </div>
  </article>;
}
