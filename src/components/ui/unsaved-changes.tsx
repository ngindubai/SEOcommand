...
export function UnsavedChanges({ dirty, saving, onDiscard, onSave }: { dirty: boolean; saving: boolean; onDiscard: () => void; onSave: () => void }) {
  ...
  return <Modal open={open} onClose={() => { setOpen(false); pending.current = null; setAwaitingSave(false); }} title="Unsaved website settings"><div className="absolute inset-0 bg-black/30" /><div className="absolute left-4 right-4 top-1/4 mx-auto max-w-md rounded-xl border border-border bg-card p-6 shadow-xl"><h2 className="text-lg font-bold">Save your changes?</h2><p className="mt-3 text-sm text-muted">You have unsaved settings for this website. Save them before continuing, or discard this draft.</p><div className="mt-5 flex flex-wrap gap-2"><Button disabled={saving} onClick={() => { setAwaitingSave(true); onSave(); }}>Save and continue</Button><Button disabled={saving} onClick={() => { onDiscard(); proceed(); }}>Discard changes</Button><Button disabled={saving} onClick={() => { pending.current = null; setOpen(false); setAwaitingSave(false); }}>Stay here</Button></div>{awaitingSave && !saving && dirty && <p className="mt-3 text-sm text-muted">Complete any validation errors in the form before continuing.</p>}</div></Modal>;
}
