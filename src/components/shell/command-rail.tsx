...
import { GLOBAL_NAV, RESEARCH_NAV, SITE_NAV, SCAN_CENTRE, TECHNICAL_SECONDARY, KEYWORD_SECONDARY, BACKLINK_SECONDARY, type NavItem } from "@/lib/nav";
    ...
    >
      <aside className={cn("absolute inset-y-0 left-0 flex flex-col border-r border-border bg-card transition-[width,box-shadow] duration-200 ease-out motion-reduce:transition-none", expanded ? "w-64" : "w-16", expanded && !pinned && "shadow-[12px_0_32px_-12px_rgba(32,24,18,0.18)]")} aria-label="Main navigation">
        <div className="flex h-20 shrink-0 items-center overflow-hidden px-3">
          <Link href="/portfolio" onClick={() => { setScope("portfolio"); finishNavigation(); }} aria-label="SEO Command dashboard" className="flex h-10 shrink-0 items-center rounded-md">
        ...
        <nav id="command-navigation" className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto overflow-x-hidden px-2 py-2" aria-label="Dashboard and tools">
          {items.map(({ href, label, icon: Icon, group }) => {
            ...
            return <Link key={`${group}:${href}`} href={`${href}${isSite ? suffix : ""}`} onNavigate={finishNavigation} aria-label={label} aria-current={active ? "page" : undefined} className={cn(rowClass, active ? "border-purple/15 bg-rail-selected text-purple" : "border-transparent text-muted hover:bg-workspace hover:text-ink")}>
        ...
        <div className="mt-2 flex shrink-0 flex-col gap-1 border-t border-border px-2 py-3">
          ...
          {[{ href: "/settings", label: "Settings", icon: Settings }, { href: "/action-centre", label: "Action centre", icon: ListChecks }].map(({ href, label, icon: Icon }) => {
