...
import { hrefWithScope, siteIdFromLocation } from "@/lib/site-context";
...
export function ContextBar() {
  ...
  const scopePicker = <select aria-label="Website or portfolio scope" value={scope} onChange={(event) => { const next = event.target.value; setScope(next); router.push(next === "portfolio" || next.startsWith("group:") ? `/portfolio?scope=${encodeURIComponent(next)}` : `/portfolio?site=${encodeURIComponent(next)}`); }} className="max-w-full rounded border border-border bg-card px-2 py-1 text-xs font-medium text-ink">
  ...
  if (!websiteMode && (pathname === "/research" || pathname.startsWith("/domain-research") || pathname.startsWith("/keyword-research"))) {
    return (
      <div className="border-b border-border bg-card">
        <div className="flex min-h-11 flex-wrap items-center gap-3 px-4 py-2 sm:px-6">
          {scopePicker}
  ...
  if (!websiteMode) return <div className="flex min-h-14 items-center gap-3 border-b border-border bg-card px-4 py-2 sm:px-6">{scopePicker}{activeDomain && <Link href={`/sites/${encodeURIComponent(activeDomain.id)}`} className="text-xs font-semibold text-purple">Website overview</Link>}</div>;
  if (!activeDomain || !siteId || requestedSiteId !== siteId) return null;
  ...
  return (
    <div className="border-b border-border bg-card">
      <div className="flex min-h-14 items-center gap-3 px-4 sm:px-6">
        ...
        {scopePicker}
        <div className="hidden min-w-0 flex-1 items-center gap-1.5 text-2xs text-muted xl:flex" aria-label="Breadcrumb">
          <Link href="/portfolio?scope=portfolio" className="hover:text-ink">All websites</Link>
          <ChevronRight className="h-3 w-3" />
          {primaryGroup && <><Link href={`/portfolio?scope=${encodeURIComponent(`group:${primaryGroup.id}`)}`} className="truncate hover:text-ink">{primaryGroup.name}</Link><ChevronRight className="h-3 w-3" /></>}
      ...
      {!pathname.includes("/settings") && (
        <nav className="flex gap-1 overflow-x-auto px-4 sm:px-6" aria-label="Website workspace">
          {navItems.map((item) => {
            ...
            const href = item.label === "Overview" ? `/sites/${siteId}` : hrefWithScope(item.href, siteId);
