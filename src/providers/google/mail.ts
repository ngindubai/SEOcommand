...
export function mailMessage(input: { id: string; from: string; to: string[]; subject: string; text: string; attachment?: Buffer }) {
  ...
  if (input.attachment) lines.push(`--${boundary}`, 'Content-Type: application/pdf; name="seo-report.pdf"', 'Content-Disposition: attachment; filename="seo-report.pdf"', "Content-Transfer-Encoding: base64", "", input.attachment.toString("base64").match(/.{1,76}/g)!.join("\r\n"));
  lines.push(`--${boundary}--`);
  return lines.join("\r\n");
}
/** Called only by an explicitly approved message action or an enabled report schedule. No automatic HTTP retries. */
export async function sendMail(input: { id: string; to: string[]; subject: string; text: string; attachment?: Buffer }) {
  ...
  const response = await fetch("https://gmail.googleapis.com/gmail/v1/users/me/messages/send", { method: "POST", headers: { authorization: `Bearer ${token}`, "content-type": "application/json" }, body: JSON.stringify({ raw }), signal: AbortSignal.timeout(30000) });
...
export async function readMailThread(threadId: string, correspondent: string) {
  ...
  return (body.messages ?? []).slice(-50).flatMap((message) => {
    ...
    return [{ id: message.id, from: correspondent, date: message.internalDate ? new Date(Number(message.internalDate)).toISOString() : null, subject: headers.find((h) => h.name.toLowerCase() === "subject")?.value ?? "", text: text(payload) || "No plain-text content supplied by Gmail." }];
  });
}
