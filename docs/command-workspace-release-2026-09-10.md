...
264 tests passed across 57 files, including a rehearsal of all migrations against a populated PostgreSQL-compatible test database, access boundaries, concurrent writes/claims, schedule permissions, cost review, retained history, URL safety and collector response handling. TypeScript, ESLint and production build pass. Representative desktop/phone browser checks cover selected-site navigation, page details/watch controls, grouped investigations, saved answers and reviewed plans. Provider collectors are tested with fixtures; routine QA makes no paid provider calls.
